 ...
 * @returns {object}
 */
export const CalendarHeader = ({ monthName, year, onMonthClick, onYearClick, next, previous }) =>
	Div({ class: 'flex items-center justify-center space-x-2 relative min-h-12 text-sm font-medium' }, [
		Button({ click: onMonthClick, variant: 'ghost', 'aria-label': 'Select month' }, monthName || '[[monthName]]'),
		Button({ click: onYearClick, variant: 'ghost', 'aria-label': 'Select year' }, year?.toString() || '[[current.year]]'),
